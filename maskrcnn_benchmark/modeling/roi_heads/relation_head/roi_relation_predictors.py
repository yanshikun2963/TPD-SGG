...
class PrototypeEmbeddingNetwork(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.training:
            ...
            # Shared warmup counter for TEPA / CPTR.
            # It must not depend on TEPA only, otherwise CPTR-only ablation never starts.
            if self.use_tepa or self.use_cptr:
                self._comp_iter += 1
            ...
            l21 = torch.norm(torch.norm(simil_mat, p=2, dim=1), p=1) / (num_rel_cls * num_rel_cls)
            ...
            gamma2 = 7.0
            predicate_proto_a = predicate_proto.unsqueeze(dim=1).expand(-1, num_rel_cls, -1)
            predicate_proto_b = predicate_proto.detach().unsqueeze(dim=0).expand(num_rel_cls, -1, -1)
            ...
            dist_loss = torch.max(
                torch.zeros(num_rel_cls, device=predicate_proto.device),
                -topK_proto_dis + gamma2
            ).mean()  # Lr_euc = max(0, -(d-) + gamma2)
            ...
            gamma1 = 1.0
            rel_rep_expand = rel_rep.unsqueeze(dim=1).expand(-1, num_rel_cls, -1)  # r
            ...
            mask_neg = torch.ones(rel_labels.size(0), num_rel_cls, device=rel_rep.device)
